...
s32 FirmwareDownloadBT(PADAPTER padapter, PRT_MP_FIRMWARE pFirmware)
{
	...
	if (rtw_is_file_readable(rtw_fw_mp_bt_file_path) == _TRUE) {
		...
		rtStatus = rtw_retrieve_from_file(rtw_fw_mp_bt_file_path, FwBuffer, FW_8723B_SIZE);
...
#endif /* CONFIG_FILE_FWIMG */
	{
		...
		pBTFirmwareBuf = (u8 *)rtl8723b_fw_bt_img_array;
		BTFirmwareLen = rtl8723b_fw_bt_img_array_length;
	...
	if (PlatformEFIORead1Byte(padapter, REG_FWHW_TXQ_CTRL + 2) & BIT6)
		bRecover = _TRUE;
...
s32 rtl8723b_FirmwareDownload(PADAPTER padapter, BOOLEAN  bUsedWoWLANFw)
{
	...
	case FW_SOURCE_HEADER_FILE:
		if (bUsedWoWLANFw) {
			...
			if (pwrpriv->wowlan_mode) {
				pFirmware->szFwBuffer = array_mp_8723b_fw_wowlan;
				pFirmware->ulFwLength = array_length_mp_8723b_fw_wowlan;
			...
			if (pwrpriv->wowlan_ap_mode) {
				pFirmware->szFwBuffer = array_mp_8723b_fw_ap;
				pFirmware->ulFwLength = array_length_mp_8723b_fw_ap;
				RTW_INFO(" ===> %s fw: %s, size: %d\n",
			...
			if (padapter->registrypriv.mp_mode == 0) {
				pFirmware->szFwBuffer = array_mp_8723b_fw_nic;
				pFirmware->ulFwLength = array_length_mp_8723b_fw_nic;
				RTW_INFO("%s fw: %s, size: %d\n", __FUNCTION__, "FW_NIC", pFirmware->ulFwLength);
			} else {
#ifdef CONFIG_MP_INCLUDED
				odm_read_firmware_mp_8723b_fw_mp(&pHalData->odmpriv, (u8 *)&pFirmware->szFwBuffer, &pFirmware->ulFwLength);
	...
	if (padapter->registrypriv.mp_mode == 1) {
		...
		RTW_INFO("rtl8723b_FirmwareDownload go to FirmwareDownloadBT !\n");
...
exit:
	...
	RTW_INFO(" <=== rtl8723b_FirmwareDownload()\n");
	return rtStatus;
}
...
static void read_chip_version_8723b(PADAPTER padapter)
{
	...
	pHalData->version_id.ICType = CHIP_8723B;
...
)
{
	...
	RTW_INFO("%s hwinfo[EEPROM_Voltage_ADDR_8723B] =%02x\n", __func__, hwinfo[EEPROM_Voltage_ADDR_8723B]);
...
)
{
	...
	if (!AutoLoadFail) {
		pHalData->crystal_cap = hwinfo[EEPROM_XTAL_8723B];
		if (pHalData->crystal_cap == 0xFF)
			pHalData->crystal_cap = EEPROM_Default_CrystalCap_8723B;	   /* what value should 8812 set? */
	} else
		pHalData->crystal_cap = EEPROM_Default_CrystalCap_8723B;
}
...
)
{
	...
	if (_FALSE == AutoLoadFail)
		pHalData->eeprom_thermal_meter = PROMContent[EEPROM_THERMAL_METER_8723B];
	else
		pHalData->eeprom_thermal_meter = EEPROM_Default_ThermalMeter_8723B;
	...
	IN		BOOLEAN		AutoloadFail)
{
	...
	if (!AutoloadFail || (Adapter->registrypriv.RegPwrTrimEnable == 1)) {
		pHalData->EEPROMRFGainOffset = PROMContent[EEPROM_RF_GAIN_OFFSET];
		RTW_INFO("AutoloadFail =%x,\n", AutoloadFail);
	...
	RTW_INFO("EEPRORFGainOffset = 0x%02x\n", pHalData->EEPROMRFGainOffset);
}
...
void fill_txdesc_force_bmc_camid(struct pkt_attrib *pattrib, u8 *ptxdesc)
{
	if ((pattrib->encrypt > 0) && (!pattrib->bswenc)
		...
		SET_TX_DESC_EN_DESC_ID_8723B(ptxdesc, 1);
		SET_TX_DESC_MACID_8723B(ptxdesc, pattrib->bmc_camid);
	}
}
#endif
void fill_txdesc_bmc_tx_rate(struct pkt_attrib *pattrib, u8 *ptxdesc)
{
	...
	SET_TX_DESC_TX_RATE_8723B(ptxdesc, MRateToHwRate(pattrib->rate));
	...
	u8 *pbuf)
{
	...
	if (pxmitframe->frame_tag == DATA_FRAMETAG) {
		...
		SET_TX_DESC_TX_RATE_8723B(pbuf, MRateToHwRate(pattrib->rate));
...
static s32 c2h_handler_8723b(_adapter *adapter, u8 id, u8 seq, u8 plen, u8 *payload)
{
	s32 ret = _SUCCESS;

	switch (id) {
	case C2H_CCX_TX_RPT:
		CCX_FwC2HTxRpt_8723b(adapter, payload, plen);
		break;
	default:
		ret = _FAIL;
		break;
...
static void dump_mac_txfifo_8723b(void *sel, _adapter *adapter)
{
	u32 rqpn, rqpn_npq;
...
void GetHwReg8723B(PADAPTER padapter, u8 variable, u8 *val)
{
	...
	case HW_VAR_DUMP_MAC_TXFIFO:
		dump_mac_txfifo_8723b(val, padapter);
		break;
	default:
...
void rtl8723b_start_thread(_adapter *padapter)
{
	...
	if (IS_ERR(xmitpriv->SdioXmitThread)) {
		RTW_ERR("%s: start rtl8723bs_xmit_thread FAIL!!\n", __func__);
...
int rtl8723b_GpioFuncCheck(PADAPTER adapter, u8 gpio_num)
{
	...
	if (IS_HARDWARE_TYPE_8723B(adapter) == _FAIL) {
		if((gpio_num > 15) || (gpio_num < 4)) {
			RTW_INFO("%s The gpio number does not included 4~15.\n",__FUNCTION__);
			ret = _FAIL;
                }
	}
